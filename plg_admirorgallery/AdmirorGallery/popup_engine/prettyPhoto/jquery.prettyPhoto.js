...
------------------------------------------------------------------------- */
(function($) {
	jQuery.prettyPhoto = {version: '2.5.6'};
	jQuery.fn.prettyPhoto = function(settings) {
		...
		// Cached selectors
                ...
                if(jQuery('.pp_overlay').size()==0) {
                _buildOverlay(); // If the overlay is not there, inject it!
                } else {
                // ensure the cached selectors are initialized
                $pp_pic_holder = jQuery('.pp_pic_holder');
                $ppt = jQuery('.ppt');
                $pp_overlay = jQuery('div.pp_overlay');
                }
		...
		jQuery(window).scroll(function(){scrollPos = _getScroll();_centerOverlay();_resizeOverlay();});
		jQuery(window).resize(function(){_centerOverlay();_resizeOverlay();});
		...
		jQuery(this).each(function(){
			$(this).bind('click',function(){
				...
				if(theGallery){
					jQuery('a[rel*='+theGallery+']').each(function(i){
		...
		*/
		jQuery.prettyPhoto.open = function(gallery_images,gallery_titles,gallery_descriptions) {
			...
			$pp_pic_holder.find('.currentTextHolder').text((setPosition+1) + settings.counter_separator_label + jQuery(images).size());
			...
			if(movie_width.indexOf('%') != -1 || movie_height.indexOf('%') != -1){
				movie_height = parseFloat((jQuery(window).height() * parseFloat(movie_height) / 100) - 100);
				movie_width = parseFloat((jQuery(window).width() * parseFloat(movie_width) / 100) - 100);
				percentBased = true;
			}
		...
		*/
		...
		function _buildOverlay(){
			...
			$('a.pp_close').bind('click',function(){jQuery.prettyPhoto.close();return false;});
