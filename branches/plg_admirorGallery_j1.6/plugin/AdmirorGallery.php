...
class plgContentAdmirorGallery extends JPlugin {
    ...
    function onContentPrepare (&$context,&$row, &$params, $limitstart) {
        ...
        require_once (dirname(__FILE__).'/AdmirorGallery/AdmirorGallery/classes/agGallery.php');
        require_once (dirname(__FILE__).'/AdmirorGallery/AdmirorGallery/classes/jquery.scroll.js.php');
        //CreateGallerys
        if (preg_match_all("#{AdmirorGallery[^}]*}(.*?){/AdmirorGallery}|{AG[^}]*}(.*?){/AG}#s", $row->text, $matches, PREG_PATTERN_ORDER)> 0) {
            ...
            foreach ($matches[0] as $match) {
                ...
                include (dirname(__FILE__).'/AdmirorGallery/AdmirorGallery/templates/'.$AG->params['template'].'/index.php');
