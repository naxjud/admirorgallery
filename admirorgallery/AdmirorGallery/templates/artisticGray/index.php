...
if(!function_exists("imageInfo")){
  include(JPATH_BASE .DS.'/plugins/content/AdmirorGallery/templates/artisticGray/imageInfo.php');
}
if(!function_exists("fileRoundSize")){
  include(JPATH_BASE .DS.'/plugins/content/AdmirorGallery/templates/artisticGray/fileRoundSize.php');
}
$doc->addStyleSheet($joomla_site_path.'/plugins/content/AdmirorGallery/templates/artisticGray/template.css');
...
<tbody>
<tr>
<td class="agArtistic_galleryHolder_gray">
';
...
if (!empty($images))
{
	foreach ($images as $imagesKey => $imagesValue)
	{
	    ...
	    $agArtistic_imageInfo_array=imageInfo(JPATH_BASE .DS.$rootFolder.$imagesFolder_name.'/'.$imagesValue);
			...
			$html .= '<span class="agArtistic_item"><a href="'.$joomla_site_path.$rootFolder.$imagesFolder_name.'/'.$imagesValue.'" class="'.$cssClass.'" rel="'.$rel.'" '.$tempTag.' title="';
